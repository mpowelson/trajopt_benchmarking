...
int main(int argc, char** argv)
{
  ...
  std::ofstream ofs;
  std::string path = ros::package::getPath("trajopt_examples") + "/solver_comparison_case2_output.csv";
  ...
  for (int num_step : num_steps)
  {
    for (double box_x : pick_points_x)
    {
      for (double box_y : pick_points_y)
      {
        ...
        std::cout << "Iter: " << ind << "/" << num_iter << " Pick point: " << box_x << ", "
                  << box_y
                  /*<< "  Place point: " << place_point */
                  << " steps_per_phase: " << steps_per_phase << '\n';

        // Save results to file
        ofs << ind << ',' << num_iter << ',' << box_x << ',' << box_y << /*',' << place_point <<*/ ','
            << steps_per_phase << ',' << solver << ',' << pick_time << ',' << place_time << ',' << pick_collision << ','
